// src/components/features/production/production-view.tsx
'use client'
import { useState } from 'react'
import { useBonsProduction, useCreateBonProduction, useClotureBP } from '@/lib/hooks/use-production'
...
import { Plus, Factory, CheckCircle, Eye } from 'lucide-react'
...
export function ProductionView() {
  ...
  const { mutate: cloture }  = useClotureBP()

  const bps      = data?.data.data ?? []
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle={`${paginate?.total ?? 0} ordre${(paginate?.total ?? 0) > 1 ? 's' : ''} de fabrication`}
        actions={
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border">
                {['Numéro', 'Produit', 'Site', 'Date', 'Cible', 'Produit', 'Taux', 'Coût', 'Statut', ''].map((h) => (
            ...
            <tbody className="divide-y divide-surface-border">
              {bps.map((bp: BonProduction) => (
                <tr key={bp.id} className="hover:bg-surface-muted/60 transition-colors">
                  <td className="px-4 py-3"><span className="ref-code">{bp.numero}</span></td>
                  <td className="px-4 py-3 font-medium text-steel-800 max-w-[160px] truncate" title={bp.produit?.designation}>
                  ...
                  <td className="px-4 py-3"><span className="amount">{formatMGA(bp.cout_total)}</span></td>
                  <td className="px-4 py-3">
                    <Badge variant={getStatutColor(bp.statut.valeur)} dot>{bp.statut.libelle}</Badge>
                  </td>
      ...
      <Dialog open={showCreate} onClose={() => setShowCreate(false)} title="Nouveau bon de production" size="md">
